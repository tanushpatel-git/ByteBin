...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
    return (
        <>
            ...
            <aside
                className={`fixed lg:static top-0 bottom-0 left-0 z-50 w-[280px] bg-[var(--bg-base)] border-r border-[var(--border-subtle)]/50 flex flex-col shrink-0 shadow-[4px_0_24px_rgba(0,0,0,0.02)] transition-transform duration-300 ease-in-out h-full ${
                    isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
                }`}
            >
                {/* Logo */}
                <div className="h-[72px] px-6 lg:px-8 flex items-center justify-between">
                    ...
                    {onClose && (
                        <button
                            onClick={onClose}
                            className="p-1.5 rounded-xl text-[var(--text-muted)] hover:text-[var(--text-main)] lg:hidden"
                ...
                {/* Navigation */}
                <nav className="flex-1 px-5 py-4 lg:py-6 space-y-7 overflow-y-auto scrollbar-hide">
