...
const NOTIFICATIONS = [
  ...
  },
  {
    id: "n2",
    title: "PR #42 merged",
    body: "feat/workspace-switcher → main",
    time: "18m ago",
    dot: "bg-[#D3ACFF]",
  },
  {
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            {visible.length === 0 ? (
              <p className="text-[13px] text-[#9CA3AF] text-center py-8">All caught up!</p>
            ) : (
              <div className="divide-y divide-[#F8F5F2]">
                {visible.map((n) => (
                  <div
                    key={n.id}
                    className="flex items-start gap-3 px-4 py-3 hover:bg-[#FDFCFB] group transition-colors"
                  >
                    ...
                    >
                      <X size={11} className="text-[#9CA3AF] hover:text-[#EF4444]" />
                    </button>
                  </div>
                ))}
              </div>
